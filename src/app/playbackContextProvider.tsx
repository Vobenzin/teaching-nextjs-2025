...
function handleNextPlaybackStatus(prev: PlaybackStatus): PlaybackStatus {
  if (prev.isShuffled) {
    ...
    if (isLast && prev.isRepeat == false) {
      return { ...prev, 
        isPlaying: false, 
        playbackStart: null };
    }
    else if (isLast && prev.isRepeat) {
    return {
      ...prev,
      shufflePosition: 0,
  ...
  else if(isLastSong && prev.isRepeat == false ){
...
export function PlaybackContextProvider({
  ...
  const { isPlaying, progress, isShuffled, queue, playbackStart, shuffleOrder,shufflePosition , currentSongIndex, isRepeat} =
    playbackStatus;
